...
export default class TaskBar extends PIXI.Container {
  ...
  createMovePanel () {
    ...
    const title = new PIXI.Text('Moves', {
      fontSize: 10,
      fill: 0x999999
    })
    ...
    num.name = 'num'
    num.position.set(
      back.width / 2 - num.width / 2,
      back.height / 2 - num.height / 2
    )
  ...
  updateMoves (num) {
    ...
    textMove.text = num
    textMove.position.set(
      this.movePanel.width / 2 - textMove.width / 2,
      this.movePanel.height / 2 - textMove.height / 2
    )
  }
