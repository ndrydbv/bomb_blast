...
export default class Field extends PIXI.Container {
  ...
  create () {
    ...
    back.drawRoundedRect(
      0,
      0,
      this.cols * this.cell.w,
      this.rows * this.cell.h,
      5
    )
    this.addChild(back)
  }
  ...
  fieldHandler (e) {
    ...
    if (e.target instanceof Tile) {
      for (let i = 0; i < this.tiles.length; i++) {
        if (this.tiles[i].timeLine?.isActive()) return
      }
  ...
  createTiles (tiles) {
    ...
    tiles.forEach((tile) => {
      const r = tiles.reduce((acc, cur) =>
        cur.row > acc.row && cur.col === tile.col ? cur : acc
      )
  ...
  getTile (pos) {
    return this.tiles.find(
      (tile) => tile.options.row === pos.row && tile.options.col === pos.col
    )
  }
