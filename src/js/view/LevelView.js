...
export default class LevelView extends View {
  ...
  addParticles (tile) {
    ...
    const particles = taskTile
      ? [tile.createParticleTask(taskTile)]
  ...
  updatePosition () {
    this.taskBar.position.set(
      -this.taskBar.width / 2,
      -app.size.height / 2 + 10
    )
    this.field.position.set(
      -this.field.width / 2,
      -this.field.height / 2 + this.taskBar.height / 2 + 5
    )
  }
