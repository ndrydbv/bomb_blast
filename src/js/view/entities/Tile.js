...
export default class Tile extends PIXI.Container {
  ...
  animMove () {
    this.timeLine
      .from(this, Math.abs(this.options.pivot.y) * this.speed, {
        y: this.y + this.options.pivot.y,
        ease: 'linear'
      })
      .to(this.sp, 0.05, {
        y: this.sp.y + 5,
        height: this.sp.height - 10,
        width: this.sp.width + 5,
        ease: 'linear'
      })
      .to(this.sp, 0.15, {
        y: this.sp.y - 5,
        height: this.sp.height + 5,
        width: this.sp.width - 5,
        ease: 'linear'
      })
      .to(this.sp, 0.1, {
        y: this.sp.y,
        height: this.sp.height,
        width: this.sp.width,
        ease: 'linear'
      })
  }
