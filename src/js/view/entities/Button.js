...
export default class Button extends PIXI.Container {
  ...
  create () {
    ...
    this.w =
      this.w > text.width + this.padding.x * 2
        ? this.w
        : text.width + this.padding.x * 2
    this.h =
      this.h > text.height + this.padding.y * 2
        ? this.h
        : text.height + this.padding.y * 2
    text.position.set(
      this.w / 2 - text.width / 2,
      this.h / 2 - text.height / 2
    )
    ...
    const back = new PIXI.Sprite(
      Texture.linearGradientVertical(this.w, this.h, this.gradients[this.color])
    )
    back.mask = mask
  ...
  pointerOver () {
    this.timeLine.clear()
    this.timeLine.to(this.outlineFilter, 0.2, { outerStrength: 2 })
  }

  pointerOut () {
    this.timeLine.clear()
    this.timeLine.to(this.outlineFilter, 0.2, { outerStrength: 0 })
  }
