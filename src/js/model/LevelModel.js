...
export default class LevelModel extends Model {
  constructor (level) {
    ...
    this.setting = JSON.parse(
      JSON.stringify(require(`../levels/${this.level}.json`))
    )
  ...
  getSimpleTilesIdDouble (num) {
    ...
    return Lib.arrShuffle(
      Object.values(TILES.simple)
        .map((tile) => tile.id)
        .reduce(
          (acc, cur, i, arr) =>
            acc.concat(Array(number(Math.floor(num / arr.length))).fill(cur)),
          []
        )
    )
  }
  ...
  createTiles () {
    ...
    for (let row = 0; row < this.rows; row++) {
      for (let col = 0; col < this.cols; col++) {
        if (this.field[row][col] !== '0' || this.field[row][col] === 'X') { continue }
  ...
  moveTiles () {
    ...
    for (let row = this.rows - 1; row >= 0; row--) {
      for (let col = this.cols - 1; col >= 0; col--) {
        if (this.field[row][col] !== '0') continue
  ...
  createSuperTile (num, tile) {
    ...
    const groupTypes = Object.keys(minGroupSuper).sort(
      (a, b) => minGroupSuper[b] - minGroupSuper[a]
    )
  ...
  handlerTileSimple (tile) {
    ...
    const group = Lib.getGroup(
      ...
      tile.row,
      tile.col,
      field[tile.row][tile.col]
    )

    if (group.length < this.setting.tiles.minGroup) return false
